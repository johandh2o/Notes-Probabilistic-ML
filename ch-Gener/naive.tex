...
Let $D=(y_i,x_i)_{i=1}^n=(\vb{y},\vb{X})$ be a \red{random sample}, then the \red{MLE} estimators of $\theta\in\Theta, \delta\in\Delta$ are:
\begin{align*}
...
Let $q(\delta|\alpha)$ be the PDF of $\delta$ and $g(\theta|\beta)$ be the PDF of $\theta$, with \red{hyperparameters} $\alpha\in A, \beta\in B$. Let $D=(y_i,x_i)_{i=1}^n=(\vb{y},\vb{X})$ be a \red{random sample}, then the \red{MAP} estimators of $\theta\in\Theta, \delta\in\Delta$ are:
\begin{align*}
...
It is an asymptotic approximation (in some cases exact) of the \red{predictive posterior distribution}:
\begin{align*}
...
If all features are Bernoulli random variables, $X_{i,j}|(Y_i=c_k)\overset{iid}{\sim}\text{Ber}(\theta_{j,k})$, and $\beta$ follows a \red{Beta distribution}, $\theta\sim\text{Bet}(\beta_0,\beta_1),\ \beta_{0,1}>0$, MAP estimator are:
\begin{equation*}
